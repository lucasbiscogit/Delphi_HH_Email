...
begin
 ...
 // Mensagem para Fechar
   ...
   mensagem:= ' O Caixa Foi Aberto... ' + #13    // #13 Pular Linha   ' INFORMAÇÂO '
  ...
  with ADOAbreCaixa do
   ...
   // Verificando se o Caixa esta Aberto
      ...
      // Usando o ADOQUERY Criado
        qry.Close;
        qry.SQL.Clear;
        qry.SQL.Add(' select top 1 ID, VL_INICIO from TB_CAIXA_ABREFECHA where DATA_FINAL is null and HORA_FINAL is null');
        qry.Open;
          if (qry.RecordCount = 0) then
            ...
            begin
              ...
              lblCaixa.Caption:= lblCaixa.Caption + ' Caixa Numero: '+ IntToStr(qry.Fields[0].Value); // Colocando o numero do caixa
              edtValor.Text:= 'R$ ' + FloatToStr(qry.Fields[1].Value);
            end;
      finally
        FreeAndNil(qry);
      end;
...
procedure TF_CaixaAbrir.TimerPiscaLabelTimer(Sender: TObject);
begin
  with lblCaixa do
    begin

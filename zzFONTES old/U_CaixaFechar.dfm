object F_CaixaFechar: TF_CaixaFechar
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'FECHAR CAIXA'
  ClientHeight = 357
  ClientWidth = 221
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object pnlCampos: TPanel
    Left = 0
    Top = 0
    Width = 221
    Height = 295
    Align = alClient
    BevelInner = bvLowered
    TabOrder = 0
    ExplicitWidth = 379
    ExplicitHeight = 460
    object lblCaixa: TLabel
      Left = 14
      Top = 10
      Width = 193
      Height = 23
      Alignment = taCenter
      Caption = 'NAO EXISTE CAIXA ABERTO'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clRed
      Font.Height = -13
      Font.Name = 'Arial Rounded MT Bold'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Visible = False
      WordWrap = True
    end
    object Label1: TLabel
      Left = 50
      Top = 43
      Width = 27
      Height = 13
      Caption = 'Caixa'
    end
    object Label2: TLabel
      Left = 50
      Top = 92
      Width = 85
      Height = 13
      Caption = 'Data Fechamento'
    end
    object Label3: TLabel
      Left = 50
      Top = 141
      Width = 36
      Height = 13
      Caption = 'Usuario'
    end
    object Label4: TLabel
      Left = 50
      Top = 190
      Width = 54
      Height = 13
      Caption = 'Valor Inicial'
    end
    object Label5: TLabel
      Left = 50
      Top = 239
      Width = 51
      Height = 13
      Caption = 'Valor Total'
    end
    object DBEdit1: TDBEdit
      Left = 50
      Top = 61
      Width = 121
      Height = 21
      Color = clInfoBk
      DataField = 'ID'
      DataSource = DSFecharCaixa
      Enabled = False
      TabOrder = 0
    end
    object DBEdit2: TDBEdit
      Left = 50
      Top = 110
      Width = 121
      Height = 21
      Color = clInfoBk
      DataField = 'DATA_FINAL'
      DataSource = DSFecharCaixa
      Enabled = False
      TabOrder = 1
    end
    object edtVlInicial: TDBEdit
      Left = 50
      Top = 208
      Width = 121
      Height = 21
      Color = clInfoBk
      DataField = 'VL_INICIO'
      DataSource = DSFecharCaixa
      Enabled = False
      TabOrder = 2
    end
    object DBEdit4: TDBEdit
      Left = 50
      Top = 257
      Width = 121
      Height = 21
      Color = clInfoBk
      DataField = 'VL_FECHAMENTO'
      DataSource = DSFecharCaixa
      Enabled = False
      TabOrder = 3
    end
    object edtUser: TEdit
      Left = 50
      Top = 159
      Width = 121
      Height = 21
      Alignment = taCenter
      Color = clInfoBk
      Enabled = False
      TabOrder = 4
    end
  end
  object pnlBotoes: TPanel
    Left = 0
    Top = 295
    Width = 221
    Height = 62
    Align = alBottom
    BevelInner = bvLowered
    TabOrder = 1
    ExplicitTop = 460
    ExplicitWidth = 379
    object BitBtn1: TBitBtn
      Left = 22
      Top = 16
      Width = 75
      Height = 25
      Caption = '&Cancelar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      TabOrder = 1
      TabStop = False
      OnClick = BitBtn1Click
    end
    object btnFechar: TBitBtn
      Left = 110
      Top = 16
      Width = 75
      Height = 25
      Caption = '&Fechar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      TabOrder = 0
      OnClick = btnFecharClick
    end
  end
  object DSFecharCaixa: TDataSource
    DataSet = ADOFecharCaixa
    Left = 16
    Top = 112
  end
  object ADOFecharCaixa: TADOQuery
    Connection = DMPrincipal.ADOConnectionPrincipal
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select *  from TB_CAIXA_ABREFECHA where DATA_FINAL is null and H' +
        'ORA_FINAL is null')
    Left = 16
    Top = 56
    object ADOFecharCaixaID: TAutoIncField
      Alignment = taCenter
      FieldName = 'ID'
      ReadOnly = True
    end
    object ADOFecharCaixaDATA_INICIO: TDateTimeField
      FieldName = 'DATA_INICIO'
    end
    object ADOFecharCaixaDATA_FINAL: TDateTimeField
      Alignment = taCenter
      FieldName = 'DATA_FINAL'
    end
    object ADOFecharCaixaHORA_INICIO: TWideStringField
      FieldName = 'HORA_INICIO'
      Size = 16
    end
    object ADOFecharCaixaHORA_FINAL: TWideStringField
      FieldName = 'HORA_FINAL'
      Size = 16
    end
    object ADOFecharCaixaUSER_INICIO: TIntegerField
      FieldName = 'USER_INICIO'
    end
    object ADOFecharCaixaUSER_FINAL: TIntegerField
      FieldName = 'USER_FINAL'
    end
    object ADOFecharCaixaVL_INICIO: TFloatField
      Alignment = taCenter
      FieldName = 'VL_INICIO'
      DisplayFormat = 'R$ ###,##0.00'
    end
    object ADOFecharCaixaVL_FECHAMENTO: TFloatField
      Alignment = taCenter
      FieldName = 'VL_FECHAMENTO'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaVLTOTALRECEBIDO: TFloatField
      FieldName = 'VLTOTALRECEBIDO'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaVLTOTALSAIDA: TFloatField
      FieldName = 'VLTOTALSAIDA'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaVLTOTALGERAL: TFloatField
      FieldName = 'VLTOTALGERAL'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaVLSAIDACONTAS: TFloatField
      FieldName = 'VLSAIDACONTAS'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaSALDO_DINHEIRO: TFloatField
      FieldName = 'SALDO_DINHEIRO'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaSALDO_BOLETO: TFloatField
      FieldName = 'SALDO_BOLETO'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaSALDO_CHEQUE: TFloatField
      FieldName = 'SALDO_CHEQUE'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaSALDO_CARTAO: TFloatField
      FieldName = 'SALDO_CARTAO'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaSAIDA_DINHEIRO: TFloatField
      FieldName = 'SAIDA_DINHEIRO'
      DisplayFormat = '###,##0.00'
    end
    object ADOFecharCaixaSAIDA_CHEQUE: TFloatField
      FieldName = 'SAIDA_CHEQUE'
      DisplayFormat = '###,##0.00'
    end
  end
  object TimerPiscaLabel: TTimer
    Enabled = False
    Interval = 500
    OnTimer = TimerPiscaLabelTimer
    Left = 120
    Top = 8
  end
end

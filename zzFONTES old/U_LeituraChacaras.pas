...
// Verificando o que esta sendo Feito -- Adicionando  ou Editando Para Dar a mensagem
    if ADOTabela.State in [dsInsert] then          //*********** ADOQUERY = ADOTABELS **************************************
       ...
       // GRAVACAO DOS DADOS NO BANCO
        CDSLeitura.First;
           while not CDSLeitura.Eof do  // Faça em quanto não for o final do Registro
             begin
              ADOQLeituraChacaras.Insert;
  ...
  // Usando o ADOQUERY Criado
    ...
    QTDEReg:=  qry.RecordCount;
     if qry.RecordCount = 0 then // Nao Tem Registro
        begin
          mensagem:= ' Nao tem registro para realizar a operacao. ' + #13    // #13 Pular Linha   ' INFORMAÇÂO '
...
begin
 ...
 DescPrincipal:= 'MES_REFERENCIA';
   ...
   statBarPadrao.Panels[2].Text:= ' Mes Referencia Contém: '+ edtValorCampo.Text;
...
begin
 ...
 DescPrincipal:= 'NOME_CHACARA';
   ...
   edtValorCampo.SetFocus;
   statBarPadrao.Panels[2].Text:= ' Nome Chacara Contém: '+ edtValorCampo.Text;
   strSql:= 'SELECT * FROM ' + nomeTabela;
   strSql:= strSql + ' WHERE '+ DescPrincipal + ' LIKE ' +#39+'%'+ edtValorCampo.Text+'%'+#39;
...
begin
 ...
 DescPrincipal:= 'NOME_PROPRIETARIO';
   ...
   edtValorCampo.SetFocus;
   statBarPadrao.Panels[2].Text:= ' Nome Proprietario Contém: '+ edtValorCampo.Text;
   strSql:= 'SELECT * FROM ' + nomeTabela;
   strSql:= strSql + ' WHERE '+ DescPrincipal + ' LIKE ' +#39+'%'+ edtValorCampo.Text+'%'+#39;
...
begin
    ...
    if edtValorCampo.Text <> '' then
        begin
            statBarPadrao.Panels[2].Text:=' Pesquisa por Leitura Atual, Código =  '+edtValorCampo.Text;
            Executar:= sentencaSQL;
            Executar:= exibePanels;
            Executar:= navegacao;
            Executar:= habilitaBotoes;
...
begin
    ...
    if edtValorCampo.Text <> '' then
        begin
            statBarPadrao.Panels[2].Text:=' Pesquisa por Registro da Chacara, Código =  '+edtValorCampo.Text;
            Executar:= sentencaSQL;
            Executar:= exibePanels;
            Executar:= navegacao;
            Executar:= habilitaBotoes;
...
begin
    ...
    if edtValorCampo.Text <> '' then
        begin
            statBarPadrao.Panels[2].Text:=' Pesquisa por Numero da Chacara, Código =  '+edtValorCampo.Text;
            Executar:= sentencaSQL;
            Executar:= exibePanels;
            Executar:= navegacao;
            Executar:= habilitaBotoes;

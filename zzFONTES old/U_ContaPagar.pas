...
// Metodo Para Execuções
  ...
  TF_ContaPagar = class(TForm)
    ...
    Label12: TLabel;
    Label1: TLabel;
    ADOQContaPagarID: TAutoIncField;
    ADOQContaPagarSTATUS: TStringField;
    ADOQContaPagarINATIVO: TStringField;
    ADOQContaPagarDATA_INC: TDateTimeField;
    ADOQContaPagarDATA_ALT: TDateTimeField;
    ADOQContaPagarDATA_HAB: TDateTimeField;
    ADOQContaPagarNUMDOC: TStringField;
    ADOQContaPagarID_CLIENTE: TIntegerField;
    ADOQContaPagarID_CLASSCONT: TIntegerField;
    ADOQContaPagarNUMPARCELA: TStringField;
    ADOQContaPagarTOTALPARCELA: TStringField;
    ADOQContaPagarVLTOTAL: TBCDField;
    ADOQContaPagarVALORPARCELA: TBCDField;
    ADOQContaPagarLIQUIDADO: TStringField;
    ADOQContaPagarDTPAGMENTO: TWideStringField;
    ADOQContaPagarFORMA_PGTO: TStringField;
    ADOQContaPagarVLPAGO: TBCDField;
    ADOQContaPagarDATA_VENC: TDateTimeField;
    ADOQContaPagarOBS: TMemoField;
    ...
    ADOForncID: TAutoIncField;
    ADOForncSTATUS: TStringField;
    ADOForncINATIVO: TStringField;
    ADOForncDATA_INC: TDateTimeField;
    ADOForncDATA_ALT: TDateTimeField;
    ADOForncDATA_HAB: TDateTimeField;
    ADOForncRAZAO_SOCIAL: TStringField;
    ADOForncNOME_FANTASIA: TStringField;
    ADOForncCNPJ: TStringField;
    ADOForncIE: TStringField;
    ADOForncTELEF_COMERCIAL: TStringField;
    ADOForncATENDENTE: TStringField;
    ADOForncSITE: TStringField;
    ...
    procedure ComboFornecEnter(Sender: TObject);
...
procedure TF_ContaPagar.btnAlterarClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.btnAnteriorClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.btnFinalClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.btnGravarClick(Sender: TObject);
var
...
procedure TF_ContaPagar.btnIncluirClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.btnInicioClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.btnOkClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.btnProximoClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.btnReverterClick(Sender: TObject);
begin
  // Verificando o que esta sendo Feito -- Adicionando  ou Editando Para Dar a mensagem
    if ADOTabela.State in [dsInsert] then               //*********** ADOQUERY = ADOTABELS **************************************
        begin
...
procedure TF_ContaPagar.btnSairClick(Sender: TObject);
begin
  Close; // Fechar a Tela
end;


procedure TF_ContaPagar.ComboFornecEnter(Sender: TObject);
begin
ComboFornec.Color:= clMoneyGreen;
end;

procedure TF_ContaPagar.ComboFornecExit(Sender: TObject);
begin
ComboFornec.Color:= clWindow;
end;

procedure TF_ContaPagar.ComboContabilEnter(Sender: TObject);
begin
ComboContabil.Color:= clMoneyGreen;
end;

procedure TF_ContaPagar.ComboContabilExit(Sender: TObject);
begin
...
if Trim(RichOBS.Text) = '' then
  begin
    RichOBS.Text:= edtNumDoc.Text + '_' + ComboFornec.Text +'_'+ ComboContabil.Text + '_TLCONTA_RECEBER';
  end;
end;

procedure TF_ContaPagar.Descrio1Click(Sender: TObject);
begin
   ADOTabela.Close;
   ADOTabela.SQL.Clear;
...
procedure TF_ContaPagar.edtDTVencEnter(Sender: TObject);
begin
edtDTVenc.Color:= clMoneyGreen;
end;

procedure TF_ContaPagar.edtDTVencExit(Sender: TObject);
begin
edtDTVenc.Color:= clWindow;
end;

procedure TF_ContaPagar.edtNumDocEnter(Sender: TObject);
begin
edtNumDoc.Color:= clMoneyGreen;
end;

procedure TF_ContaPagar.edtNumDocExit(Sender: TObject);
begin
edtNumDoc.Color:= clWindow;
end;

procedure TF_ContaPagar.edtNumParcelaEnter(Sender: TObject);
begin
   edtNumParcela.Color:= clMoneyGreen;
end;

procedure TF_ContaPagar.edtNumParcelaExit(Sender: TObject);
begin
...
procedure TF_ContaPagar.edtNumParcelaKeyPress(Sender: TObject; var Key: Char);
begin
If not( key in['0'..'9',#08] ) then
  key:=#0;
end;

procedure TF_ContaPagar.edtTotParcelaEnter(Sender: TObject);
begin
  edtTotParcela.Color:= clMoneyGreen;
end;

procedure TF_ContaPagar.edtTotParcelaExit(Sender: TObject);
begin
// IF PARA NAO DEIXAR COM ZERO OU EM BRANCO
   if (Trim(edtTotParcela.Text) = '' ) OR (Trim(edtTotParcela.Text) = '0' ) then
      begin
     ...
     //  MensageAppMesBox:= '   ABRIR  CAIXA  ';
      ...
      // MENSAGEM
          if Application.MessageBox(PChar(mensagem), PChar(MensageAppMesBox) , MB_YESNO+MB_ICONINFORMATION) = mrYes then
            ...
            // verificando se o campo esta vazio
              if ComboFornec.Text = '' then
                begin
                  ComboFornec.KeyValue:= 0;
                  IDCLINT:= ComboFornec.KeyValue;
                end
              else
                begin
                   IDCLINT:= ComboFornec.KeyValue;
...
procedure TF_ContaPagar.edtTotParcelaKeyPress(Sender: TObject; var Key: Char);
begin
If not( key in['0'..'9',#08] ) then
  key:=#0;
end;

procedure TF_ContaPagar.edtValorCampoKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TF_ContaPagar.edtValorParcelaEnter(Sender: TObject);
begin
   edtValorParcela.Color:= clMoneyGreen;
end;

procedure TF_ContaPagar.edtValorParcelaExit(Sender: TObject);
var
VLParc, Tot: Currency;
TotalParce: Integer;
begin
...
procedure TF_ContaPagar.edtVlTotalEnter(Sender: TObject);
begin
  edtVlTotal.Color:= clMoneyGreen;
end;

procedure TF_ContaPagar.edtVlTotalExit(Sender: TObject);
begin
  edtVlTotal.Color:= clWindow;
end;

procedure TF_ContaPagar.ExcluirUMRegClick(Sender: TObject);
...
procedure TF_ContaPagar.FormActivate(Sender: TObject);
begin
  ADOTabela:= ADOQContaPagar;   // Definindo o Valor da Variavel que ira Receber o ADOQuery   //*********** ADOQUERY = ADOTABELS ******************
  nomeTabela:= 'TB_CONTAPAGAR'; // Definindo a Tabela para trabalhar
  nomeJanela:= 'Cadastro de Contas a Pagar';  // Nome que ira aparecer
  GridLista.DataSource:= DSContaPagar;    //Colocando o Data Source no Grid
  DescPrincipal:= 'NUMDOC';  // Define o Campo que ira Fazer a Pesquisa Padrao por Desc.

    ModoEdicao:= F_Principal.ADOQLogin.FieldByName('CONTPAGAR_INC').AsAnsiString +
                 F_Principal.ADOQLogin.FieldByName('CONTPAGAR_ALT').AsAnsiString +
                 F_Principal.ADOQLogin.FieldByName('CONTPAGAR_EXC').AsAnsiString +
                 F_Principal.ADOQLogin.FieldByName('CONTPAGAR_MAR').AsAnsiString +
                 F_Principal.ADOQLogin.FieldByName('CONTPAGAR_IMP').AsAnsiString +
                 F_Principal.ADOQLogin.FieldByName('CONTPAGAR_INA').AsAnsiString +
                 F_Principal.ADOQLogin.FieldByName('CONTPAGAR_HAB').AsAnsiString;

// Caption dos ApplicationMessageBox  Do EXECUTAR SentencaSQL
    MensageAppMesBox:= 'CONTAS A PAGAR';
...
procedure TF_ContaPagar.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 // Fechando as Conexoes
  ADOQContaPagar.Close;
  ADOClassContabil.Close;
  ADOFornc.Close;
end;

procedure TF_ContaPagar.FormKeyPress(Sender: TObject; var Key: Char);
begin
 //ENTER No Lugar de TAB
    if key = #13 then
...
procedure TF_ContaPagar.GridListaDblClick(Sender: TObject);
begin
       // ADOTabela.Locate('ID',ADOTabelaID.Value,[]);
        PageControlCadastro.ActivePage:= TabSheetFicha;
end;

procedure TF_ContaPagar.GridListaDrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
   GridZebrado (TDBGrid(Sender).DataSource.DataSet.RecNo, TDBGrid(Sender), Rect, Column, State); // Procedure para Pintar
end;

procedure TF_ContaPagar.GridListaKeyPress(Sender: TObject; var Key: Char);
begin
//ENTER No Lugar de TAB
    if key = #13 then
       Perform(WM_NEXTDLGCTL,0,0);
...
procedure TF_ContaPagar.GridZebrado(RecNo: Integer; Grid: TDBGrid; Rect: TRect;
...
procedure TF_ContaPagar.HabBotoesPesDesCheck;
begin
...
procedure TF_ContaPagar.HabCheckDesBotoes;
begin
...
procedure TF_ContaPagar.Inativo1Click(Sender: TObject);
begin
   ADOTabela.Close;
   ADOTabela.SQL.Clear;
   ADOTabela.SQL.Add (strSql);
...
procedure TF_ContaPagar.OrdeCodigoClick(Sender: TObject);
begin
   ADOTabela.Close;
   ADOTabela.SQL.Clear;
   ADOTabela.SQL.Add (strSql);
...
procedure TF_ContaPagar.OrdeDTAlteracaoClick(Sender: TObject);
begin
   ADOTabela.Close;
   ADOTabela.SQL.Clear;
   ADOTabela.SQL.Add (strSql);
...
procedure TF_ContaPagar.OrdeDTHabilitacaoClick(Sender: TObject);
begin
   ADOTabela.Close;
   ADOTabela.SQL.Clear;
   ADOTabela.SQL.Add (strSql);
...
procedure TF_ContaPagar.OrdeDTInclusaoClick(Sender: TObject);
begin
   ADOTabela.Close;
   ADOTabela.SQL.Clear;
   ADOTabela.SQL.Add (strSql);
...
procedure TF_ContaPagar.PesCodigoClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.PesqPPAlteracaoClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.PesqPPInclusaoClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.PesqRegBloqueadosClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.PesqToRegClick(Sender: TObject);
begin
...
procedure TF_ContaPagar.PesquisaPorCodigo;
...
procedure TF_ContaPagar.PesquisaPorDescricao;
begin
  edtValorCampo.SetFocus;
...
procedure TF_ContaPagar.RGPesquisaGenericaClick(Sender: TObject);
begin
  edtValorCampo.SetFocus;
end;

procedure TF_ContaPagar.RichOBSEnter(Sender: TObject);
begin
  RichOBS.Color:= clMoneyGreen;
end;

procedure TF_ContaPagar.RichOBSExit(Sender: TObject);
begin
...
if Trim(RichOBS.Text) = '' then
  begin
    RichOBS.Text:= edtNumDoc.Text + ' ' + ComboFornec.Text +' '+ ComboContabil.Text + ' TLCONTA_RECEBER';
  end;
end;

procedure TF_ContaPagar.SetExecutar(const Value: TExecutar);
begin
...
procedure TF_ContaPagar.Status1Click(Sender: TObject);
begin
   ADOTabela.Close;
   ADOTabela.SQL.Clear;
   ADOTabela.SQL.Add (strSql);
...
procedure TF_ContaPagar.VerificaSeTaRecebido;
var
Liquidd: string;
begin
 ADOQContaPagar.Open;

 Liquidd:= ADOQContaPagarLIQUIDADO.Value;
  ...
  if Liquidd =  'S' then  // Se estiver Pago fazer
   ...
   if (ADOQContaPagarDATA_VENC.Value < Date) then    // Verificando se esta Vencida
       begin

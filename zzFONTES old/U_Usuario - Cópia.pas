...
// Verificando se o Campo Login esta Vazio
  if Trim(edtLogin.Text) = '' then
     begin
       Beep;
       ShowMessage('O Campo LOGIN, não pode Ficar Vazio.');
       edtLogin.SetFocus;
       Abort;
     end;
             ...
             if ADOUsuario.RecordCount <> 0 then
              begin
                mensagem:= ' Este Login Ja Existe no Sistema ' + #13
                            +' Código que esta o Cadastro: '+ IntToStr(ADOUsuarioID.Value);
...
procedure TF_Fornecedor.edtSenhaEnter(Sender: TObject);
begin
...
procedure TF_Fornecedor.edtSenhaExit(Sender: TObject);
var
...
procedure TF_Fornecedor.edtLoginExit(Sender: TObject);
begin
...
procedure TF_Fornecedor.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   ADOPerfil.Close;
end;
...
procedure TF_Fornecedor.Login1Click(Sender: TObject);
begin
 ...
 if F_PesqString.RadioGroup1.ItemIndex = 0 then
    begin
        statBarPadrao.Panels[2].Text:= ' Nome Contém: ' + F_PesqString.edtValor.Text;
        strSql:=' SELECT * FROM ' + nomeTabela;

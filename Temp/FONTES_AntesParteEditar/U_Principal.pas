...
// S se clicou em algum botao que é para sair do campo sem o erro;
  sairsemerro: string;
...
begin
  ...
  MensageAppMesBox:= 'E-mails que Receberão o Hora-Hora';
...
begin
 ...
 //Pegando o e-mail inicialmente digitado e colocando já pra inserção.
...
if btnAdicionar.Tag = 1 then
  ...
  //Controle de botoes
    ...
    RadioButtonBaseOutUser.Visible:= False;
    RadioButtonPorCliente.Visible:= False;
    RadioButtonManual.Visible:= False;
      ...
      with qry do
           begin
             ...
             if (qry.RecordCount = 0) then // Se não encontrar dados
                begin
                   mensagem:= ' Não Existe Arquivo de Excel, ' + #13
...
{$REGION 'Se for por Cliente'}
 if btnAdicionar.Tag = 2 then
   begin
     ...
     RadioButtonBaseOutUser.Visible:= True;
     RadioButtonPorCliente.Visible:= True;
     RadioButtonManual.Visible:= True;
   ...
   case RGPesquisaGeral.ItemIndex of
    ...
    try
    // Criando Um ADOQUERY Em Tempo de Execução para buscar clientes
      qry:= TADOQuery.Create(nil);
      qry.Connection:= con_Geral;
...
//Verificar se é registro unico
  ...
  // pnlGridEscluir.Height:= 253;
   ...
   ComboExcluirArqExcCL.Enabled:= False;
   ComboExcluirGCarteiraCL.Enabled:= False;
...
{$REGION 'Busca para alimentar Combo Cliente'}
   ...
   Except
     on E:Exception do
        ...
        End;
   End;
{$ENDREGION 'Busca para alimentar Combo Cliente'}
end;
...
{$REGION 'Se estiver adicionando Manual'}
    if btnAdicionar.Tag = 1 then
      ...
      //Verificar os campos pra não salvar em branco
        ...
        if Trim(edtEmail.Text) = '' then
          begin
             mensagem:= 'O Campo E-MAIL não pode Ficar vazio';
             Application.MessageBox(PChar(mensagem),PChar(MensageAppMesBox),MB_OK+ MB_ICONINFORMATION);
             edtEmail.SetFocus;
             Exit;
          end
        ...
        if Trim(ComboArqExcel.Text) = '' then
          begin
             mensagem:= 'O Campo ARQUIVO DO EXCEL não pode Ficar vazio';
             Application.MessageBox(PChar(mensagem),PChar(MensageAppMesBox),MB_OK+ MB_ICONINFORMATION);
        ...
        if Trim(ComboCarteira.Text) = '' then
          begin
             mensagem:= 'O Campo CARTEIRA não pode Ficar vazio';
             Application.MessageBox(PChar(mensagem),PChar(MensageAppMesBox),MB_OK+ MB_ICONINFORMATION);
...
{$ENDREGION 'Se estiver adicionando Manual e Nao tem Arquivo'}
 ...
 if btnAdicionar.Tag = 2 then
   begin
      ...
      //Tirar o foco do combo caso não saia.
   ...
   // Criando Um ADOQUERY Em Tempo de Execução  Para pegar os Arquivos de Excel
       ...
       with qryArqExcel do
          begin
            ...
            while not eof do
               ...
               // Criando Um ADOQUERY Em Tempo de Execução  da ultima parte do processo
      ...
      Begin
        ...
        Except
           ...
           Begin
            ...
            //  lblConexao.Caption:= 'Falha na Conexão BD';
 ...
 // Hint por coluna  Grid Aba principal - Aba Pesquisa
   GridLista.ShowHint := True;
    case GridLista.MouseCoord(X, Y).X of  {Coluna posicionada }
      1 : GridLista.Hint := 'Código Gerado pelo sistema';
...
begin
  ...
  // Criando um Data Source em tem de execução
     ...
     //strsql:= 'select distinct HH_Excel, Grupo_Carteira, Cliente from hh.Lista_Email order by Cliente';
      ...
      with AdoQ_Grid do
          begin
            ...
            Open;
            //First;
            if (AdoQ_Grid.RecordCount = 0) then // Se não encontrar dados
               begin
                  mensagem:= ' Não Existe Arquivos Cadastrado, ' + #13
                           +'Favor Verificar com Ad. Do Banco.';
                  Application.MessageBox(PChar(mensagem),PChar(MensageAppMesBox),MB_OK+MB_ICONINFORMATION);
